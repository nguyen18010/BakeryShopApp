...
public  class SanPhamAdapter extends BaseAdapter {
    ...
    private void showEditDialog(SanPham tt) {
        // ✅ Reset URI mỗi lần mở form sửa để tránh dính ảnh cũ
    ...
    // ✅ Hàm cập nhật thông tin sản phẩm — ĐÃ SỬA KHẮC PHỤC LỖI
    private void updateSanPham(SanPham tt, EditText editMasp, EditText editTensp, EditText editDongia, EditText editMota, EditText editGhichu, EditText editSoluongkho, Spinner editMansp) {
        ...
        String newMansp = ((NhomSanPham) editMansp.getSelectedItem()).getMa();

        // ✅ Nếu có chọn ảnh mới → dùng ảnh mới, nếu không thì giữ ảnh cũ
        byte[] newAnh;
        if (selectedImageUri != null) {
            newAnh = getBytesFromUri(selectedImageUri);
        } else {
            newAnh = tt.getAnh(); // giữ ảnh cũ
        }
        ...
        values.put("maso", newMansp);
        values.put("anh", newAnh); // ✅ luôn lưu ảnh hiện tại (mới hoặc cũ)
    ...
    private void openDrawableImagePicker(ImageView imgBacSi) {
        final String[] imageNames = {"banh1","banh2","banh3","banh4","banh5","banh6","banh7","banh8","banh9","banh10","banh11","banh12","banh13","banh14","banh15","banh16","banh17","banh18","banh19","banh20","banh21","banh22","banh23","banh24"};
