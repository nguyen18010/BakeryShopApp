...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void insertDanhGia(DanhGiaSanPham danhGia) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();

        // Thêm dữ liệu vào ContentValues
        values.put("masp", danhGia.getMasp());
        values.put("id_chitiet", danhGia.getId_chitiet());
        values.put("noidung", danhGia.getNoidung());
        values.put("tendn", danhGia.getTendn());
        values.put("sao1", danhGia.getSao1());
        values.put("sao2", danhGia.getSao2());
        values.put("sao3", danhGia.getSao3());
        values.put("sao4", danhGia.getSao4());
        values.put("sao5", danhGia.getSao5());

        // Chèn dữ liệu vào bảng DanhGia
        long id = db.insert("DanhGia", null, values);
        if (id == -1) {
            Log.e("Database Error", "Failed to insert review");
        } else {
            Log.d("Database", "Review inserted successfully with ID: " + id);
        }

        db.close(); // Đóng cơ sở dữ liệu
    }
    public boolean isReviewExists(String idChitiet) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM DanhGia WHERE id_chitiet = ?", new String[]{idChitiet});
