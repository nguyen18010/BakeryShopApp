...
public class Database extends SQLiteOpenHelper {
    ...
    public Database(Context context) {
        super(context, "banhang.db", null, 1); // Mặc định tên và phiên bản
    }
        ...
        if (cursor.moveToFirst()) {
            do {
               int id = cursor.getInt(0); // id_dathang
                ...
                String trangthai = cursor.getString(6); // ngaydathang
                // Tạo đối tượng Order và thêm vào danh sách
                orders.add(new Order(id, tenKh, diaChi, sdt, tongThanhToan, ngayDatHang,trangthai));
